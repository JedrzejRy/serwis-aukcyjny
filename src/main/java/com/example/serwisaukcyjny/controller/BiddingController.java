...
public class BiddingController {
    ...
    @PostMapping("/{id}")
    public String handleCreate(@ModelAttribute("bidding") @Valid CreateBiddingForm form, @PathVariable Long id, Principal principal, RedirectAttributes redirectAttributes) {

        User loggedUser = userService.findByLogin(principal.getName()).get();
        Auction currentAuction = auctionService.findByID(id);
        Bidding newBidding = BiddingMapper.ToEntity(loggedUser, currentAuction, form.getPrice());
        ...
        if (newBidding.getPrice().intValue() > currentAuction.getMinimumPrice().intValue()
            ...
            biddingService.save(newBidding);
